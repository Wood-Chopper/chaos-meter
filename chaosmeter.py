...
def main():
	...
	metric_value = compute_metric(graph, metric, exclude)
	print(json.dumps(metric_value, indent=4))


def compute_metric(graph, metric, exclude) -> any:
	...
	if metric is None or metric == 'in-degree':
		return highlight_larger_in_degree()
	if metric is None or metric == 'out-degree':
		return highlight_larger_out_degree()
	if metric is None or metric == 'centrality-degree':
		return highlight_larger_centrality_degree()
